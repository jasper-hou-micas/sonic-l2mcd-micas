        ...
        BOOL           sigchange)
{
    ...
    {
       L2MCD_VLAN_LOG_ERR(mcgrp_vport->vir_port_id,"%s(%d) mcgrp_mbrshp is NULL for phy_port_id, may be is BUG! ", FN, LN);
       return;
    }
        ...
        BOOL          sigchange)
{
     ...
     * 2. send update to l2mcsync to delete this port
     */
     ...
     * the inherited ports to l2mcsync 
     */
    ...
    {
        if(mcgrp_entry && mcgrp_entry->pims_num_sg_join_ports > 0)
        {
            /* has pims (S,G) join need to update membership for all SG's */
            l2mcd_sync_pims_upd_inherit_ports_to_sg(mcgrp_entry, mcgrp_mbrshp, vid, phy_ifindex,
                    mcgrp_entry->group_address.afi, FALSE/*del*/,ivid);
        ...
        BOOL           sigchange)
{
    ...
    if (sigchange) {
         ...
         * then we should not send port delete to l2mcsync for this SGV
         */
        ...
        void         *src_array)
{
    ...
    BOOLEAN              mbrship_destroyed = FALSE;
    ...
    else if (IS_MLD_CLASS(mcgrp))
    {
        ...
        for (i = 0; i < num_srcs; i++)
        {
            mcast_set_ipv6_addr(&tmp_addr, mld_src_array);
            L2MCD_VLAN_LOG_DEBUG(vir_port_id, "%s:%d:[vlan:%d] GA:%s Src:%s", FN, LN, vir_port_id, mcast_print_addr(group_address), mcast_print_addr(&tmp_addr));
        }
    }
                ...
                else 
                {
                    for (j = FILT_INCL; j <= FILT_EXCL; j++)
                    {
                        ...
                        if (igmpv3_src_temp && (igmpv3_src_temp->is_remote !=is_remote))
                        {
                            ...
                            L2MCD_VLAN_LOG_INFO(vir_port_id, "%s:%d:[vlan:%d] rmt1:%d filter:%d", FN,LN,vir_port_id,is_remote, j);
                            l2mcd_system_group_entry_notify(group_address, &src_temp, vir_port_id, phy_port_id, TRUE, TRUE);
                        }
                    }
                }
