...
int receive_igmp_packet (IP_PARAMETERS  *sptr_ip_parameters)
{
    ...
    char *if_name = portdb_get_ifname_from_portindex(multicast->source_port);
    if (if_name && l2mcd_is_peerlink(if_name) &&
 ...
 ** parameters passed.                                                                    **
 **---------------------------------------------------------------------------------------*/
// the last query use the default 5 seconds response time,
...
#error "Byte order not define"
    ...
    rx_pkt_msg.ip_param.source_address = source_address;
    ...
    rx_pkt_msg.ip_param.vrf_index = 1;
    rx_pkt_msg.ip_param.rx_phy_port_number = physical_port;
    if (version == IGMP_VERSION_3)
        rx_pkt_msg.ip_param.data = sptr_igmpv3_packet;
    else
        rx_pkt_msg.ip_param.data = sptr_igmp_packet;

    rx_pkt_msg.ip_param.total_length = igmp_packet_size; // + IP_ROUTER_ALERT_OPTION_LENGTH;
