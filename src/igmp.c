...
MCGRP_CLASS  *mcgrp_vrf_alloc (UINT32 afi, VRF_INDEX  vrf_index)
{
    ...
    mcgrp->mcgrp_wtid_lmq = WheelTimer_Create(MCGRP_MAX_TIME_SLOTS, mcgrp_process_wte_event, 
            NULL, WheelTimer2DLinkedList);
    L2MCD_INIT_LOG_INFO("%s %d allocated  %p %p vrf:%d lmq_wheeltimer:%lu",__FUNCTION__, __LINE__, mcgrp, pIgmp0, vrf_index,mcgrp->mcgrp_wtid_lmq);
    L2MCD_INIT_LOG_INFO("%s Done. mcgrp alloc for afi:%d vrf:%d",__FUNCTION__, afi,vrf_index);
    return mcgrp;
}
        ...
        MCGRP_PORT_ENTRY  *mcgrp_pport)
{
    ...
    while (mcgrp_entry)
    {
        ...
        // Notify Phy_port del to l2mcsync before destroying group membership entry.
        ...
        MCGRP_MBRSHP  *mcgrp_mbrshp) 
{
        ...
        if (mcgrp_mbrshp->host_present[version] > 0)
        {
            if (mcgrp_mbrshp->host_present[version] <= curr_time)
            {
                ...
                if (mcgrp_mbrshp->grp_compver == version)
                {
                    ...
                    if (mcgrp_pport == NULL)
                    {
                        L2MCD_VLAN_LOG_ERR(vir_port_id,"%s(%d) mcgrp_vport is NULL for phy_port_id, Illegal access is prohibited! ", FN, LN);
                        return;
                    }
    ...
    if (mcgrp_mbrshp->static_mmbr == TRUE)
    {
            ...
            if (mcgrp_mbrshp->lmq_timer <= curr_time)
            {
                ...
                if (mcgrp_mbrshp->retx_cnt > 1)
                {
                    ...
                    if (IS_IGMP_CLASS(mcgrp))
                    {
                        if (mcgrp_pport == NULL)
                        {
                            L2MCD_VLAN_LOG_ERR(vir_port_id,"%s(%d) mcgrp_vport NULL for phy_port_id, stop igmp send! ", FN, LN);
                            return;
                        }
                        igmp_send_group_query(mcgrp, mcgrp_mbrshp,
                                vir_port_id,
        ...
        UINT32        mcgrp_op)
{
    ...
    if(mcgrp_op == MCGRP_ADD_GROUP) {
         ...
         * check before we send the delete to l2mcsync
         */
