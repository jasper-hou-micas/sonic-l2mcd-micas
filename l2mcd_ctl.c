...
void l2mcd_ctl_send_pkt(char *buf, char *ifname)
{
    ...
    // Read data into g_l2mcd_test_buf1
    while (i < 2000 && !feof(pkt_fp))
    {
        int c = fgetc(pkt_fp);
        if (c == EOF)
        {
        ...
        g_l2mcd_test_buf1[i++] = (char)c;
    }
    for (j=0;j<i;j+=2)
    {
        if (j + 1 < i) { // Ensure we do not go out of bounds
            sprintf(str, "0x%c%c", g_l2mcd_test_buf1[j], g_l2mcd_test_buf1[j + 1]);
            if (j / 2 < 2000) { // Ensure we do not go out of bounds
                g_l2mcd_test_buf2[j / 2] = strtol(str, NULL, 0);
            }
        }
    }
    // Null terminate the buffer (this is not strictly necessary for unsigned long)
    g_l2mcd_test_buf2[j / 2] = 0; // Set the next element to 0 to indicate end of valid data
    fclose(pkt_fp);
    pkt_len=i;
...
{
    printf("Usage:\n");
    printf("\tl2mcd_ctl -d <log_level: 0-6>\n");
    ...
    printf("\tl2mcd_ctl [-s <cmd_op_file>] (-c <1-20>)    [Note Custom commands]\n"); 
    printf("\tl2mcd_ctl -r <vlanid> <srcip> <grpip>  <port_name> <is_add>    [Note: ICCP notify test group]\n ");
    printf("\tl2mcd_ctl (-s <ip_pkt_hex_file>) (-v <vlan>) (-i <port_if>)\n"); 
    printf("\tl2mcd_ctl -L <ip_pkt_hex_file> <port_name>  [Note: Packet generator]\n"); 
    printf("\tl2mcd_ctl -X <vlan>      [Note: clear igmp groups   <vlanid, 0 for all>]\n"); 
    printf("\tl2mcd_ctl -S <vlan>      [Note: clear igmp stats    <vlanid, 0 for all>]\n"); 
    return;
}
...
int main(int argc, char **argv)
{
    ...
    memset(&cfg_msg, 0, sizeof(L2MCD_CONFIG_MSG));
    ...
    while ((opt = getopt(argc, argv, "r:d:v:s:p:a:c:i:L:X:S:")) != -1) 
    {
        switch (opt) 
        {
        case 'r':
