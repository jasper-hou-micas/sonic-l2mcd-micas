...
struct sock_filter g_igmp_filter[] = {
    ...
    BPF_JUMP(BPF_JMP|BPF_JEQ|BPF_K, 0x00000800, 0, 3),
    //3. Load Half-Word @4
    BPF_STMT(BPF_LD |BPF_ABS|BPF_B, 0x00000017),
...
{
    uint32_t kif;
    l2mcd_if_tree_t *l2mcd_if_tree1;
...
static void l2mcd_process_ipc_msg(L2MCD_IPC_MSG *msg, int len, struct sockaddr_un client_addr)
{
    ...
    switch(msg->msg_type)
    {
        case L2MCD_IGMP_PKT_MSG:
        {
            ...
            if (!ip_param)
            {
                L2MCD_LOG_NOTICE(" No Data for recievd IPC message L2MCD_IGMP_PKT_MSG type:%u ", msg->msg_type);
                break;
            }
            igmp_pkt = (IGMP_PACKET *) ip_param->data;
            if (!igmp_pkt)
            {
                L2MCD_LOG_NOTICE(" No Param Data for recievd IPC message L2MCD_IGMP_PKT_MSG type:%u ", msg->msg_type);
                break;
            }
        ...
        case L2MCD_LAG_MEM_TABLE_UPDATE:
        {
            ...
            for (i = 0; i < data->count; i += 2)
            {
                ...
                l2mcd_add_kif_to_if(data->ports[i+1].pnames, ifidx2, -1, NULL, ifidx, -1, -1, -1, afi);
        ...
        case L2MCD_SNOOP_CONFIG_MSG:
        {
            ...
            if (warm_reboot && vlan_node && mld_is_flag_set(vlan_node, afi, MLD_SNOOPING_ENABLED))
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    int is_mrouter = mcast_addr_any(&grp_addr);
                    int has_src    = !mcast_addr_any(&src_addr);
                    ...
                    L2MCD_LOG_NOTICE(
                        ...
                        entry->port.pnames,
                        is_mrouter ? "<empty>": entry->gaddr,
                        has_src ? entry->saddr : "<empty>");
                    ...
                    int version = mcgrp_pport->oper_version;
                    ...
                    {
                        version = has_src ? IGMP_VERSION_3 : IGMP_VERSION_2;
                    }
                    else if (afi == MLD_IP_IPV6_AFI)
                    {
                        version = has_src ? MLD_VERSION_2 : MLD_VERSION_1;
                    }
        ...
        case L2MCD_SNOOP_MROUTER_CONFIG_MSG:
        {
            ...
            if (!data)
            {
                L2MCD_LOG_ERR(" No Data for recievd IPC message L2MCD_SNOOP_MROUTER_CONFIG_MSG type:%u ", msg->msg_type);
                break;
            }
            if (msg->msg_len < sizeof(L2MCD_CONFIG_MSG) + sizeof(PORT_ATTR) * data->count)
            {
        ...
        case L2MCD_SNOOP_REMOTE_CONFIG_MSG:
        {
            ...
            if (!data)
            {
                L2MCD_LOG_NOTICE(" No Data for recievd IPC message L2MCD_SNOOP_REMOTE_CONFIG_MSG type:%u ", msg->msg_type);
                break;
            }
            if (msg->msg_len < sizeof(L2MCD_CONFIG_MSG) + sizeof(PORT_ATTR) * data->count)
            {
            ...
            if (!vlan_node || (vlan_node && !mld_is_flag_set(vlan_node, afi, MLD_SNOOPING_ENABLED)))
            {
                L2MCD_LOG_NOTICE("IGMP Snooping on vlan %d is not enable!", vlan_id);
                return;
            }
            ...
            if (remote_version != 0 && remote_version > cfg->cfg_version)
            {
                L2MCD_LOG_NOTICE("version mismatch remote is %d, vlan config version is %d", remote_version, cfg->cfg_version);
                return;
            }
        ...
        case L2MCD_SNOOP_MROUTER_REMOTE_CONFIG_MSG:
        {
            ...
            if (!data)
            {
                L2MCD_LOG_NOTICE(" No Data for recievd IPC message L2MCD_SNOOP_MROUTER_REMOTE_CONFIG_MSG type:%u ", msg->msg_type);
                break;
            }
            if (msg->msg_len < sizeof(L2MCD_CONFIG_MSG) + sizeof(PORT_ATTR) * data->count)
            {
        ...
        case L2MCD_SNOOP_STATIC_CONFIG_MSG:
        {
            ...
            if (!data)
            {
                L2MCD_LOG_NOTICE(" No Data for recievd IPC message L2MCD_SNOOP_STATIC_CONFIG_MSG type:%u ", msg->msg_type);
                break;
            }
            if (msg->msg_len < sizeof(L2MCD_CONFIG_MSG) + sizeof(PORT_ATTR) * data->count)
            {
        ...
        case L2MCD_SNOOP_CTL_MSG: /* Internal Debugging & Test */
        {
            ...
            if (!ctl_data)
            {
                L2MCD_LOG_NOTICE(" No Data for recievd IPC message L2MCD_SNOOP_CTL_MSG type:%u ", msg->msg_type);
                break;
            }
            ...
            else if (ctl_data->cmd & L2MCD_CTL_VLAN_CLEAR)
            {
                g_l2mcd_cmd_fp =  fopen(ctl_data->fname,"a+");
                if (!g_l2mcd_cmd_fp)
                {
                ...
                ctl_data = (L2MCD_CTL_MSG *) msg->data;
                vlan_id = ctl_data->vid;
                l2mcd_clear_snooping(vlan_id);
                break;
            }
            else if (ctl_data->cmd & L2MCD_CTL_VLAN_STATS_CLEAR)
            {
                g_l2mcd_cmd_fp =  fopen(ctl_data->fname,"a+");
                if (!g_l2mcd_cmd_fp)
                {
                ...
                ctl_data = (L2MCD_CTL_MSG *) msg->data;
                vlan_id = ctl_data->vid;
                l2mcd_clear_snooping_stats(vlan_id);
                break;
            }
            else
            {
...
void l2mcd_100ms_timer(evutil_socket_t fd, short what, void *arg)
{
    ...
    if (0==(g_l2mcd_stats_libev_timer%10))
    {
        ...
        mcgrp_service_wheel_timer(L2MCD_IPV6_AFI);
    }
...
void l2mcd_recv_client_msg(evutil_socket_t fd, short what, void *arg)
{
    ...
    recv_len = recvfrom(fd, (void *) buffer, sizeof(buffer), 0, (struct sockaddr *) &client_sock, &addr_len);
    if (recv_len == -1)
    {
    ...
    {
        L2MCD_LOG_INFO("Received message exceeds buffer size");
        return;
    }

    L2MCD_LOG_DEBUG("%s Rcvd message len %zd", FN, recv_len);
    l2mcd_process_ipc_msg((L2MCD_IPC_MSG *)buffer, recv_len, client_sock);
}
...
int l2mcd_mldtx_sock_init()
{
    ...
    {
      L2MCD_LOG_ERR("Failed to create TX socket for MLD Snooping");  
      g_l2mcd_mld_tx_handle=0;
      return -1;
    }
    L2MCD_INIT_LOG("Created MLD TX socket fd:%d", g_l2mcd_mld_tx_handle);
    return 0;
}
...
void l2mcd_nl_msg(evutil_socket_t fd, short what, void *arg)
{
    ...
    for (h = (struct nlmsghdr*)buf; status >= (ssize_t)sizeof(*h); ) 
    {
        ...
        if (h->nlmsg_type == RTM_NEWLINK)
        {
            ...
            if (ifName && g_port_init_done && (strstr((char*)ifName, "PortChannel") || strstr((char*)ifName, "Ethernet")))
            {
...
struct event *l2mcd_igmprx_sock_init(int *fd, char *iname)
{
    ...
    *fd = socket(sock_domain, SOCK_RAW, sock_protocol);
    ...
    if (g_l2mcd_rx_is_l2_sock)
    {
        ...
        L2MCD_INIT_LOG("g_igmp_filter setsockopt SO_ATTACH_FILTER for (%s) Failed, errno : %s"
    ...
    L2MCD_INIT_LOG("IGMP RX_Socket_Create %s fd:%d Ev:%p ev_cnt:%d",  iname, *fd, igmp_rx_event, g_l2mcd_stats_libev_no_of_sockets);
    return igmp_rx_event;
}

static struct iovec iov6[L2MCD_MM_SOCKET_BATCH_SIZE][1];
struct mmsghdr mmsg6[L2MCD_MM_SOCKET_BATCH_SIZE];
...
void l2mcd_recv_mld_msg(evutil_socket_t fd, short what, void *arg)
{
    ...
    for (int i = 0; i < L2MCD_MM_SOCKET_BATCH_SIZE; i++)
    {
        ...
        mmsg6[i].msg_hdr.msg_name = &rx_sa6[i];
        mmsg6[i].msg_hdr.msg_namelen = sizeof(rx_sa6[i]);
        mmsg6[i].msg_hdr.msg_iov = iov6[i];
        mmsg6[i].msg_hdr.msg_iovlen = 1;
        mmsg6[i].msg_hdr.msg_control = cmsgbuf6[i].buf;
        mmsg6[i].msg_hdr.msg_controllen = sizeof(cmsgbuf6[i]);
        mmsg6[i].msg_hdr.msg_flags = 0;
    }
...
struct sock_filter g_mld_filter[] = {
    ...
    // if EtherType == 0x8100 (VLAN) jump ahead
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, ETH_P_8021Q, 0, 3),

    // VLAN: load real EtherType from offset 16
    BPF_STMT(BPF_LD | BPF_H | BPF_ABS, 16),
    BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, ETH_P_IPV6, 0, 3),
    BPF_STMT(BPF_RET | BPF_K, 0x00040000),

    // non-VLAN: check if EtherType == IPv6
...
struct event *l2mcd_mldrx_sock_init(int *fd, char *iname)
{
    ...
    *fd = socket(PF_PACKET, SOCK_RAW, htons(ETH_P_ALL));
    if (*fd < 0) {
        L2MCD_LOG_ERR("Failed to create MLD RX socket: %s", strerror(errno));
        return NULL;
    }

    // 2. PACKET_AUXDATA
    if (setsockopt(*fd, SOL_PACKET, PACKET_AUXDATA, &on, sizeof(on)) < 0) {
        L2MCD_LOG_ERR("PACKET_AUXDATA set failed: %s", strerror(errno));
        close(*fd);
        return NULL;
    }
    ...
    if (setsockopt(*fd, SOL_SOCKET, SO_RCVBUF, &skmem, sizeof(skmem)) < 0) {
        L2MCD_LOG_ERR("SO_RCVBUF set failed: %s", strerror(errno));
    }

    ifindex = if_nametoindex(iname);
    if (ifindex <= 0) {
        L2MCD_LOG_ERR("Invalid ifname %s", iname);
        close(*fd);
        return NULL;
    }
    ...
    if (bind(*fd, (struct sockaddr *)&sa, sizeof(sa)) < 0) {
        L2MCD_LOG_ERR("bind failed fd:%d ifname:%s err:%s", *fd, iname, strerror(errno));
        close(*fd);
        return NULL;
    }

    L2MCD_INIT_LOG("MLD RX socket bound: ifname=%s ifindex=%d fd=%d", iname, ifindex, *fd);

    prog.filter = g_mld_filter;
    prog.len = sizeof(g_mld_filter)/sizeof(struct sock_filter);

    if (setsockopt(*fd, SOL_SOCKET, SO_ATTACH_FILTER, &prog, sizeof(prog)) < 0) {
        L2MCD_LOG_ERR("MLD BPF filter attach failed: %s", strerror(errno));
        close(*fd);
        return NULL;
    }

    mld_rx_event = l2mcd_libevent_create(
        g_l2mcd_evbase, *fd,
        EV_READ | EV_PERSIST,
        l2mcd_recv_mld_msg,
        (char *)"MLD_RX_SOCK", NULL, -1
    );

    if (!mld_rx_event) {
        L2MCD_LOG_ERR("Failed to create libevent for MLD RX socket");
        close(*fd);
        return NULL;
    }

    L2MCD_INIT_LOG("MLD RX_Socket_Create %s fd:%d Ev:%p ev_cnt:%d",
                   iname, *fd, mld_rx_event, g_l2mcd_stats_libev_no_of_sockets);

    return mld_rx_event;
}


int l2mcd_mldrx_sock_close(char *pname, int fd, struct event *mld_rx_event)
{
    if (!fd) 
    {
        return -1;
    ...
    if (mld_rx_event)
    {
        l2mcd_libevent_destroy(mld_rx_event);
        L2MCD_LOG_INFO("RX_Socket_Delete %s fd:%d ev:%p ev_cnt:%d", pname, fd, mld_rx_event,g_l2mcd_stats_libev_no_of_sockets);
    }
    else
    {
        L2MCD_LOG_INFO("Closed Socket:%d , Event not found",fd);
...
int l2mcd_nl_init()
{
    ...
    struct event *ipc_event = NULL; 
    int soc_fd = 0;
    soc_fd =  socket(AF_NETLINK, SOCK_RAW, NETLINK_ROUTE); 
    if (soc_fd < 0) 
    {
...
void mcast_global_init(UINT32 afi)
{
	...
	{
		gMulticast2.instances[IPVRF_DEFAULT_VRF_IDX] = pMulticast2;
		gMulticast2.instances_list = pMulticast2;
		gMulticast2.instances_list_end = pMulticast2;
        pMulticast2->vrf_index = IPVRF_DEFAULT_VRF_IDX;
	}
    return;
}
...
int l2mcd_system_init(int flag)
{
    ...
    rc = l2mcd_mldtx_sock_init();
    if (rc < 0)
    {
        L2MCD_LOG_ERR("l2mcd MLD TX sock init failed %d", rc);
        L2MCD_INIT_LOG("l2mcd MLD TX sock init failed %d", rc);
        return -1;
    }
