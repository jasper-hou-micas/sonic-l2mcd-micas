...
void l2mcd_clear_snooping(int vid)
{
    ...
    int count=0, ret=0;
    mcast_set_addr_default(&any_address, MCAST_IPV4_AFI);

    if (!vid)
        L2MCD_CLI_PRINT("clear igmp snooping groups for all vlans");
    else 
        L2MCD_CLI_PRINT("clear igmp snooping groups for vlan: %d", vid);
    for (vlan_node = M_AVLL_FIRST(mld_vlan_db.vdb_tree); vlan_node; vlan_node = M_AVLL_NEXT(mld_vlan_db.vdb_tree, vlan_node->node))
    {
        ...
        ret=mld_iterate_vlan_group_clear(vid, &any_address, 0 , vlan_node->type);
        L2MCD_LOG_DEBUG("clear igmp group on vlan :%d ret:%d",vlan_node->gvid,ret);
        if (ret != MLD_SUCCESS) L2MCD_CLI_PRINT("clear vlan:%d failed", vlan_node->gvid);
        count++;
    }
    L2MCD_CLI_PRINT("cleared groups on %d vlans", count);
    L2MCD_LOG_NOTICE("cleared groups on %d vlans", count);
}
...
void l2mcd_clear_snooping_stats(int vid)
{
    ...
    mcast_set_addr_default(&any_address, MCAST_IPV4_AFI);
    if (!vid)
        L2MCD_CLI_PRINT("clear igmp snooping statistics for all vlans");
    else 
        L2MCD_CLI_PRINT("clear igmp snooping snooping statistics for vlan:%d", vid);
    L2MCD_CLI_PRINT("====================================================================");
    for (vlan_node = M_AVLL_FIRST(mld_vlan_db.vdb_tree); vlan_node; vlan_node = M_AVLL_NEXT(mld_vlan_db.vdb_tree, vlan_node->node))
    {
        if (vid && vid!= vlan_node->gvid) continue;
        mcgrp_reset_mld_stats(mcgrp, vlan_node->ifindex,vlan_node->type);
        L2MCD_LOG_DEBUG("clear stats on vlan :%d ",vlan_node->gvid);
    }
    return;
}
