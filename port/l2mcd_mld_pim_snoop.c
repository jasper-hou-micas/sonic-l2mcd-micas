										...
										UINT8 afi, UINT8 add,UINT32 ivid)
{
	...
	while (mbrshp) {
		...
		while (mcgrp_src)
		{
			...
			/*
		 	 * add==1 : get each source and send port add for that SG to l2mcsync
			 * add==0 : get each source and send port del for that SG to l2mcsync
		 	 */
			if(add) {
					MLD_LOG(MLD_LOGLEVEL5, afi,
						"%s(%d):PIMS: send [%s, %s] inherit %s and sendto l2mcsync ifindex:0x%x ", 
						FN, LN,
				...
				if(!mld_snp_is_source_present_on_mbr_port(rcvd_mbrshp, mcgrp_src->src_addr.ip.v4addr, afi))
				{
					...
					MLD_LOG(MLD_LOGLEVEL6, afi,
						"%s(%d) PIMS: src %s is part of recv_mbrshp skip delete to l2mcsync ", FN, LN,
						mcast_print_addr(&mcgrp_src->src_addr));
				}

			}
			if(rc)
				L2MCD_LOG_INFO("%s(%d): update to l2mcsync failed. [%s, %s], vid:%d, add:%d, ifindex:0%x",
									   ...
									   MCGRP_ENTRY *mcgrp_entry, UINT32 phy_port_id, MADDR_ST *src_addr, int add_flag)
{
	...
	MLD_LOG(MLD_LOGLEVEL7, MLD_IP_IPV4_AFI, 
			"%s(%d) Send [%s, %s] %s count:%d phy_port:%s Rte to l2mcsync ", FN, LN, 
						...
						MCGRP_SOURCE *pims_src_entry)
{
	...
	MLD_LOG(MLD_LOGLEVEL9, MLD_IP_IPV4_AFI, 
			"%s(%d)  src:%s aged on port:%d, send update to l2mcsync", FN, LN,
			mcast_print_addr(&src_addr), mcgrp_mbrshp->phy_port_id);

	/* (S,G) port delete, update to l2mcsync */ 
