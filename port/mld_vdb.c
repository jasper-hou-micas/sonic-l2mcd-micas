...
int mld_vdb_init( )
{
	...
	L2MCD_INIT_LOG_INFO("%s VDB Init. Done #afis:%d", __FUNCTION__, afi);
    return rc;
}
		                                    ...
		                                    int add, BOOLEAN is_ve)
{
	...
	msg = grpmemnode_lookup_by_ifname(list_head, input_msg->ifname);
...
int mld_add_static_mrtr_to_pending_list(mld_vlan_node_t *vlan_node, char *if_name, int add, uint8_t afi)
{
    ...
    msg = mrnode_lookup_by_ifname(list_head, if_name);
    if(!msg) {
        if(add) {
...
mld_l2_static_group_t *grpmemnode_lookup_by_ifname(struct list *list, char *ifname)
{
    mld_l2_static_group_t *node;
    struct listnode *list_node;
    assert(list);
    LIST_LOOP(list, node, list_node)
    {
        if (strcmp(ifname,node->ifname)==0)
            return node;
    }
    return NULL;
}

mld_mrtr_t *mrnode_lookup_by_ifname(struct list *list, char *ifname)
{
    mld_mrtr_t *node;
    struct listnode *list_node;
    assert(list);
...
vdb_dpv_done:
    L2MCD_LOG_NOTICE("%s:%d:[vlan_id:%d] VDB null", FN,LN, vlan_id);
    return 0;
}
