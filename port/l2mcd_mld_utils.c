...
uint32_t mld_get_vlan_id(uint16_t vir_port_id)
{
	...
	L2MCD_LOG_INFO("%s(%d) : virport id= %d ifindex  0x%x ", __FUNCTION__, __LINE__, vir_port_id, ifindex);
    ...
    {
		L2MCD_LOG_INFO("%s(%d) ifindex:0x%x is trunk vir_port_id:%d ", FN, LN, ifindex,vir_port_id);	
		return(mld_portdb_get_ivid_from_gvid(ifindex, MLD_ROUTE_PORT));
	}
	else if(l2mcd_ifindex_is_svi(ifindex)){
		L2MCD_LOG_INFO("%s(%d) vlan_id:%d vir_port_id:%d", __FUNCTION__, __LINE__, l2mcd_ifindex_get_svi_vid(ifindex),vir_port_id);
...
{
    memcpy(comp_if_name, if_name,L2MCD_IFNAME_SIZE);
    return;
}
